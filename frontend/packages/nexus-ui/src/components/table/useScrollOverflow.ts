...
type ScrollOverflowRefs = {
  ...
  /**
   * Callback ref for the element that receives `--nx-scroll-fade-opacity`; typically a wrapper
 ...
 *
 * Sets `--nx-scroll-fade-opacity` on the wrapper element to a value between `0` (fully scrolled
 * down, no more content) and `1` (content below the fold). The fade-in distance is controlled by
 * the `--nx-scroll-fade-distance` CSS custom property on the scroll element; falls back to
