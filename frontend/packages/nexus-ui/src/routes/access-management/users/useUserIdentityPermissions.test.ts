...
describe('useUserIdentityPermissions', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('calls can_i with correct action and resource_type', async () => {
    ...
    expect(accessFetchClient.POST).toHaveBeenCalledWith('/authz/can_i', {
      body: { action: 'attach', resource_type: 'user_identity' },
    })
    expect(accessFetchClient.POST).toHaveBeenCalledWith('/authz/can_i', {
      body: { action: 'detach', resource_type: 'user_identity' },
    })
  })
  ...
  it('provides standardized tooltip messages', () => {
    ...
    expect(result.current.tooltips.attach).toContain('user_identity:attach')
    expect(result.current.tooltips.detach).toContain('user_identity:detach')
  })
