...
test.describe.skip('Test Single Step from Canvas', () => {
      ...
      [
        {
          ...
          type: 'script',
          name: 'Say Hello',
          parameters: { language: 'python', code: 'print("hello", end="")' },
        },
        {
          id: 'goodbye_node',
          type: 'script',
  ...
  test('single workflow node can be tested from the canvas using prior node data', async ({ app }) => {
    ...
    await expect(
      app.getByRole('row', { name: /Manual Trigger/i }).getByRole('gridcell', { name: /Successful/i })
    ).toBeVisible({ timeout: 15_000 })
    await expect(
      app.getByRole('row', { name: /Say Hello/i }).getByRole('gridcell', { name: /Successful/i })
    ).toBeVisible({ timeout: 15_000 })
    await expect(
      app.getByRole('row', { name: /Say Goodbye/i }).getByRole('gridcell', { name: /Successful/i })
    ).toBeVisible({ timeout: 15_000 })
  })
  ...
  test('single workflow node can be tested from the canvas using pinned data', async ({ app }) => {
    ...
    await expect(app.getByRole('row', { name: /Say Hello/i }).getByRole('gridcell', { name: /Skipped/i })).toBeVisible({
      timeout: 15_000,
    })
    await expect(
      app.getByRole('row', { name: /Say Goodbye/i }).getByRole('gridcell', { name: /Successful/i })
