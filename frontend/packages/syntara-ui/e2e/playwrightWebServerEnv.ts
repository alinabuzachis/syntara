/**
 * Real-backend E2E runs set `NEXUS_E2E_SKIP_WEB_SERVER=1` so Playwright does not start the mock
...
export function isSkipWebServerForPlaywrightTests(): boolean {
  const v = process.env.NEXUS_E2E_SKIP_WEB_SERVER?.toLowerCase().trim() ?? ''
