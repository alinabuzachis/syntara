...
import pytest
from syntara_api_client.models.group_create import GroupCreate
...
class GroupFactory(Protocol):
    ...
    def __call__(
        self, api: SyntaraApiRegistry, prefix: str | None = None, group_name: str | None = None
    ) -> tuple[UUID, str]: ...
...
def create_group() -> Generator[GroupFactory, None, None]:
    ...
    def _create_group(
        api: SyntaraApiRegistry, prefix: str | None = None, group_name: str | None = None
    ) -> tuple[UUID, str]:
...
def add_to_group(api: SyntaraApiRegistry, group_id: UUID, user_id: UUID) -> None:
    """Add a user to a group."""
    resp = api.groups.add_member(group_id=group_id, body=GroupMemberAdd(user_id=user_id))
    assert resp.status_code == HTTPStatus.CREATED


def remove_from_group(api: SyntaraApiRegistry, group_id: UUID, user_id: UUID) -> None:
