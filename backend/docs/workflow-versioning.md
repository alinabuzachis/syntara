...
- **Single published version** — only one version per workflow can be published at a time, enforced at the database level by a partial unique index
...
graph TD
    ...
    B --> C[(workflow_versions<br/>status enum)]
    B --> D[(workflows<br/>published_version)]
...
- **Router** (`src/nexus/workflows/router.py`) — publish, unpublish, restore, export, list versions, get version endpoints with RBAC enforcement
- **WorkflowService** (`src/nexus/workflows/services/workflow_service.py`) — all version lifecycle business logic: publish (with empty-nodes validation), unpublish, restore, create version, export
...
The `WorkflowVersionStatus` enum (`src/nexus/workflows/models/workflow_version.py`):

| Value | Meaning |
|-------|---------|
| `draft` | Default state for new versions. Not executable via triggers. |
| `published` | The active version. Triggered executions use this version. |
| `previously_published` | Was published, then replaced or unpublished. |
...
| `published_version` | int (nullable) | Version number of the published version, or `NULL` |
| `is_enabled` | bool | Derived from publish state — `True` when `published_version` is set |

**Design note — denormalized integer fields**: `current_version` and `published_version` are plain integers referencing version numbers, not UUID foreign keys. `current_version` is intentional denormalization: while derivable as `MAX(version)`, it enables single-column joins in the execution path, powers the optimistic concurrency check (`expected_version`), and avoids subqueries on every save and execution start. Both fields are maintained atomically by the service layer within the same transaction that creates/publishes versions.
...
erDiagram
    ...
    workflows {
        uuid id PK
        int current_version
        int published_version "nullable"
    ...
    workflow_versions {
        ...
        uuid workflow_id FK
        int version
        enum status "draft / published / previously_published"
        varchar publish_name "nullable"
...
|------------|------|---------|
| `ix_workflow_versions_single_published` | Partial unique index | Only one published version per workflow (`WHERE status = 'published' AND deleted_at IS NULL`) |
| `ck_workflows_is_enabled_published_version` | CHECK | Enforces `(published_version IS NULL) = (NOT is_enabled)` |
...
sequenceDiagram
    ...
    alt Previous version published
        WorkflowService->>DB: UPDATE prev → previously_published
    end
    WorkflowService->>DB: INSERT new published version
    WorkflowService->>DB: UPDATE workflows.published_version, is_enabled=True
...
3. Run `collect_findings()` validation on the definition
4. **Publish-only gate**: reject if `nodes` is empty — saves are allowed with empty nodes (canvas-first), but publish requires at least one step. This check lives in the service method, not the validator, to preserve the save path.
5. If the target is already published — update metadata only (`publish_name`, `change_description`)
6. If a different version is currently published — demote it to `previously_published`
7. Create a new version with status `published`; update `workflow.published_version` and `is_enabled = True`
8. Sync webhook triggers using the published version's definition
9. Atomic commit

**Why publish creates a new version record**: Publishing inserts a new `WorkflowVersion` row (a copy of the source definition) rather than flipping the status on the existing row. This is a deliberate UX decision: the version history panel sorts by `created_at`, and UX requires the published version to appear at the top of the list. An in-place status change would leave the published version buried at its original creation time. Creating a new record gives it a fresh timestamp and a new sequential version number, ensuring it sorts to the top. This also means the source draft version is preserved unchanged in the history.
...
3. Demote the published version to `previously_published`
4. Set `workflow.published_version = None`, `is_enabled = False`
...
3. Call `_create_version_record()` with the target's `workflow_definition` and `change_description = "Restored from {source_label}"`
...
- **Webhook-triggered** executions resolve the version via `Workflow.published_version` (`use_published=True`)
- **Manual/test** executions resolve via `Workflow.current_version` (`use_published=False`)
