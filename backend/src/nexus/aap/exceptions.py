...
@fastapi_exception(handler="nexus.aap.error_handlers.aap_connection_error_handler")
class AAPConnectionError(NexusError):
    """Cannot connect to AAP Controller (network error, timeout)."""


@fastapi_exception(handler="nexus.aap.error_handlers.aap_authentication_error_handler")
class AAPAuthenticationError(NexusError):
    """AAP returned 401/403 — invalid token or credentials."""


@fastapi_exception(handler="nexus.aap.error_handlers.aap_upstream_error_handler")
