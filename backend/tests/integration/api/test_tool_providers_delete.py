...
class TestToolProvidersDeleteContract:
    ...
    @pytest.mark.asyncio
    async def test_delete_provider_not_found_contract(self, jwt_client_with_provider_factory: AsyncClient) -> None:
        """Test 404 error for non-existent provider."""
        provider_id = "99999999-9999-9999-9999-999999999999"

        response = await jwt_client_with_provider_factory.delete(f"/api/v1/tool_manager/tool_providers/{provider_id}")
    ...
    @pytest.mark.asyncio
    async def test_delete_provider_invalid_uuid_contract(self, jwt_client_with_provider_factory: AsyncClient) -> None:
        """Test 400 error for invalid UUID format."""
        invalid_id = "not-a-uuid"

        response = await jwt_client_with_provider_factory.delete(f"/api/v1/tool_manager/tool_providers/{invalid_id}")
    ...
    async def test_delete_provider_soft_delete_behavior_contract(
        ...
        """
        # Delete the provider
        delete_response = await jwt_client_with_provider_factory.delete(
        ...
        list_response = await jwt_client_with_provider_factory.get("/api/v1/tool_manager/tool_providers")
    ...
    async def test_delete_provider_idempotent_contract(
        ...
        # First deletion
        response1 = await jwt_client_with_provider_factory.delete(
            f"/api/v1/tool_manager/tool_providers/{test_tool_provider.id}"
        )
        assert response1.status_code == 204

        # Second deletion attempt
        response2 = await jwt_client_with_provider_factory.delete(
            f"/api/v1/tool_manager/tool_providers/{test_tool_provider.id}"
        )
