...
@pytest.mark.asyncio
async def test_get_workflow_versions_list(jwt_client: AsyncClient) -> None:
    ...
    await jwt_client.patch(f"/api/v1/workflows/{workflow_id}", json=update_data_v2)
    ...
    await jwt_client.patch(f"/api/v1/workflows/{workflow_id}", json=update_data_v3)
...
@pytest.mark.asyncio
async def test_get_workflow_versions_empty_list(jwt_client: AsyncClient) -> None:
...
async def test_get_workflow_versions_nonexistent_workflow(
    ...
    response = await jwt_client.get(f"/api/v1/workflows/{fake_id}/versions")

    assert response.status_code == 404


@pytest.mark.asyncio
async def test_get_workflow_versions_includes_metadata(jwt_client: AsyncClient) -> None:
