...
@pytest.mark.asyncio
async def test_delete_workflow_success(jwt_client: AsyncClient) -> None:
    ...
    # Delete workflow
    response = await jwt_client.delete(f"/api/v1/workflows/{workflow_id}")

    assert response.status_code == 204


@pytest.mark.asyncio
async def test_delete_workflow_sets_deleted_at(jwt_client: AsyncClient) -> None:
    ...
    get_response = await jwt_client.get(f"/api/v1/workflows/{workflow_id}")
    assert get_response.status_code == 404


@pytest.mark.asyncio
async def test_delete_workflow_excluded_from_list(jwt_client: AsyncClient) -> None:
    ...
    create_response1 = await jwt_client.post("/api/v1/workflows", json=workflow1)
    workflow_id1 = create_response1.json()["id"]

    create_response2 = await jwt_client.post("/api/v1/workflows", json=workflow2)
    workflow_id2 = create_response2.json()["id"]

    # Delete first workflow
    await jwt_client.delete(f"/api/v1/workflows/{workflow_id1}")
...
@pytest.mark.asyncio
async def test_delete_nonexistent_workflow(jwt_client: AsyncClient) -> None:
    """Test deleting a non-existent workflow.

    Expected: 404 Not Found
    """
    fake_id = "00000000-0000-0000-0000-000000000000"
    response = await jwt_client.delete(f"/api/v1/workflows/{fake_id}")

    assert response.status_code == 404


@pytest.mark.asyncio
async def test_delete_already_deleted_workflow(jwt_client: AsyncClient) -> None:
    ...
    # First deletion
    first_delete = await jwt_client.delete(f"/api/v1/workflows/{workflow_id}")
    assert first_delete.status_code == 204

    # Try to delete again
    second_delete = await jwt_client.delete(f"/api/v1/workflows/{workflow_id}")
