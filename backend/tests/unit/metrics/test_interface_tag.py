...
class TestDetectInterface:
    ...
    def test_ui_header_present(self) -> None:
        """Request with X-Nexus-Client: ui is classified as UI."""
    ...
    def test_no_header_defaults_to_api(self) -> None:
        """Request without X-Nexus-Client header defaults to API."""
    ...
    def test_unknown_header_value_classified_as_api(self) -> None:
        """Unrecognized X-Nexus-Client values default to API."""
        scope = _make_scope(headers=[(b"x-nexus-client", b"cli")])
        assert detect_interface(scope) == INTERFACE_API

        scope = _make_scope(headers=[(b"x-nexus-client", b"unknown")])
        assert detect_interface(scope) == INTERFACE_API

    def test_empty_header_value_classified_as_api(self) -> None:
        """Empty X-Nexus-Client value defaults to API."""
        scope = _make_scope(headers=[(b"x-nexus-client", b"")])
        assert detect_interface(scope) == INTERFACE_API
...
class TestConstants:
    ...
    def test_header_name_is_lowercase(self) -> None:
        ...
        assert INTERFACE_HEADER == "x-nexus-client"
...
class TestDetectInterfaceEdgeCases:
    ...
    def test_various_header_values(self, value: bytes, expected: str) -> None:
        """Parametrised test for different X-Nexus-Client values."""
        scope = _make_scope(headers=[(b"x-nexus-client", value)])
