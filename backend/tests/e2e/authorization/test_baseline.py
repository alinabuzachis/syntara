...
class TestZeroRoleBaseline:
    ...
    def test_list_workflows_returns_empty(self, nexus_base_url: str, admin_api: NexusApiRegistry) -> None:
    ...
    def test_list_credentials_returns_empty(self, nexus_base_url: str, admin_api: NexusApiRegistry) -> None:
    ...
    def test_list_executions_returns_empty(self, nexus_base_url: str, admin_api: NexusApiRegistry) -> None:
    ...
    def test_list_projects_returns_empty(self, nexus_base_url: str, admin_api: NexusApiRegistry) -> None:
    ...
    def test_cannot_access_specific_project_workflows(self, nexus_base_url: str, admin_api: NexusApiRegistry) -> None:
...
class TestSelfScopePolicies:
    ...
    def test_can_read_own_profile(self, nexus_base_url: str, admin_api: NexusApiRegistry) -> None:
        _, username, password = create_user(admin_api, "self-read")
        user_api = api_for(nexus_base_url, username, password)
        user_api.authentication.get_current_user().assert_and_get()

    def test_cannot_read_other_user_profile(self, nexus_base_url: str, admin_api: NexusApiRegistry) -> None:
    ...
    def test_can_read_own_role_assignments(self, nexus_base_url: str, admin_api: NexusApiRegistry) -> None:
    ...
    def test_cannot_read_other_role_assignments(self, nexus_base_url: str, admin_api: NexusApiRegistry) -> None:
        ...
        assignment_id = assign_role_to_user(admin_api, proj_id, u2_id, "project-user")
        u1_api = api_for(nexus_base_url, u1_name, u1_pass)
        resp = u1_api.users.list_role_assignments(user_id=u2_id)
        if resp.is_success:
            assert resp.parsed is not None
            resource_ids = {str(r.id) for r in resp.parsed.resources}
    ...
    def test_can_update_own_profile(self, nexus_base_url: str, admin_api: NexusApiRegistry) -> None:
    ...
    def test_cannot_update_other_profile(self, nexus_base_url: str, admin_api: NexusApiRegistry) -> None:
