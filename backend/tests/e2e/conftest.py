...
Loads nexus_test_sdk.e2e.fixtures which provides:
...
These override the API-level stubs from nexus_test_sdk.e2e.factories (which
is registered globally in the plugin) for the backend/tests/e2e/ subtree, and
also override the DB-level factories from nexus_test_sdk.app.factories
(loaded by backend/tests/conftest.py) for this subtree.
"""

pytest_plugins = [
    "nexus_test_sdk.e2e.fixtures",
]
