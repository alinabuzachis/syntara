...
class TestAPI36AdminManualGroupAssignment:
    ...
    def test_local_user_manual_group_assignment(
        ...
        members_resp = nexus_api.groups.list_members(group_id=group_id)
        assert members_resp.status_code == HTTPStatus.OK
        assert members_resp.parsed is not None
        member = next(m for m in members_resp.parsed.resources if m.id == user.id)
    ...
    def test_federated_user_manual_group_assignment_and_removal(
        ...
        assert members_resp.status_code == HTTPStatus.OK
        assert members_resp.parsed is not None
        member = next(m for m in members_resp.parsed.resources if m.id == user_id)
        assert any(s.type_ == "manual" for s in (member.membership_sources or []))
        ...
        assert members_resp.status_code == HTTPStatus.OK
        assert members_resp.parsed is not None
        member_ids = {m.id for m in members_resp.parsed.resources}
