...
| 2   | **`useQueryState` uses object form with `onRetry`** (no unary `void`; use `detachPromise(query.refetch())`)              | Search for `useQueryState` in changed files; flag any using bare string title; flag `void query.` / `void .*refetch` patterns                                                                                                                                                                                                                                                                                                                                                                  |
...
| 15  | **No JavaScript unary `void` operator**                                                                                  | Search changed `.ts`/`.tsx` for `void` used as an operator (e.g. `void promise()`). ESLint `no-void` forbids it. For intentionally unawaited promises, use `detachPromise(...)` from `packages/nexus-ui/src/utils/detachPromise.ts` (not bare `.catch(() => {})` on possibly `undefined` mocks). Otherwise `await` the promise or `return` it so the caller handles errors. This rule targets the **unary** `void` operator only, not TypeScript `: void` return types.                        |
| 16  | **Prefer PF6 components over native HTML elements**                                                                      | Search changed files for native `<button>`, `<ul>`, `<ol>`, `<li>`, `<p>`, `<h1>`–`<h6>`, `<a>` elements; flag and suggest PF6 equivalents (`Button`, `List`/`ListItem`, `Content component={ContentVariants.p}`, `Title`, `Button variant="link"`). **Exceptions** (no PF6 equivalent): `<span>` (inline text — `ContentVariants.span` does not exist in PF6), `<code>` (inline code), `<div>` (generic layout containers), `<strong>`/`<em>` (inline emphasis). See table below for mapping. |

### HTML → PF6 Component Mapping

Use this reference when reviewing native HTML elements in changed files:
