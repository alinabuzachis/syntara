...
Redis is the key/value store for the Nexus platform. It serves as a cache system for event streaming and multi-client synchronization, aligned with AAP conventions (decision-records.md, 2026-01-28).
...
All Redis configuration lives in `src/nexus/core/config/base.py` under `CacheSettings`:
