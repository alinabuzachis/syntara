...
/**
 * Parse trigger label to extract type and format details
 * Examples:
 * - "Manual" -> { type: "Manual", details: null }
 * - "Manual (Requires Approval)" -> { type: "Manual", details: "Requires Approval" }
 * - "Scheduled (Interval: R/...)" -> { type: "Scheduled", details: "Starts Jan 1, 2026 at 12:00 AM, Repeats weekly, Ends Feb 6, 2026" }
 * - "Scheduled (Continuous)" -> { type: "Scheduled", details: "Continuous" }
 */
export function parseTriggerLabel(label: string): { type: string; details: string | null } {
  const match = label.match(/^([^(]+)(?:\s*\((.+)\))?$/)
  if (match) {
    const type = match[1].trim()
    const rawDetails = match[2]?.trim() || null

    if (rawDetails) {
      // Check if it's an interval string
      if (rawDetails.startsWith('Interval: ')) {
        const interval = rawDetails.substring('Interval: '.length)
        const formatted = formatIntervalDescription(interval)
        return {
          type,
          details: formatted || rawDetails,
        }
      }
      // Check if it's a cron expression
      if (rawDetails.startsWith('Cron: ')) {
        return {
          type,
          details: rawDetails,
        }
      }
      // Otherwise use as-is (e.g., "Continuous")
      return {
        type,
        details: rawDetails,
      }
    ...
    return { type, details: null }
  }
  return { type: label, details: null }
}
