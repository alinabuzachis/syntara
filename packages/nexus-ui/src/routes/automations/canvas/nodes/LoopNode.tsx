...
export function LoopNodeComponent(props: NodeProps<LoopNode>) {
  ...
  return (
    <NodeComponent className={metadata.className} enableEnd={metadata.enableEnd} disableSource nodeProps={props}>
...
function LoopNodeHandle(props: { children: React.ReactNode; id: string }) {
  return (
    <div className="group/handle relative rounded-l-4xl border-y-2 border-l-2 border-white/20 bg-white/10 px-3 py-1 text-xs hover:border-white/40 hover:bg-white/20">
      {props.children}
      <Handle
        type="source"
        id={props.id}
