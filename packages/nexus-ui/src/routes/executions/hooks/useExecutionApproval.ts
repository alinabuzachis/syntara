...
export function useExecutionApproval(executionId: string | undefined): UseExecutionApprovalResult {
  ...
  const { fetchForNode, clear } = useFetchApprovalForNode(executionId ?? '')
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- synchronizing approval state with route param change
  ...
  return { pendingApproval, handleNodeClick, clearPendingApproval }
}
