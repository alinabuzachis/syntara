...
/** Returns arrays of child activities for each nesting slot of the given activity type. */
function getNestedChildren(activity: Activity): Activity[][] {
...
function hasLegacyActivityTypes(activities: Activity[]): boolean {
  for (const activity of activities) {
    ...
    for (const nested of getNestedChildren(activity)) {
...
function activitiesEqual(a: Activity, b: Activity): boolean {
  ...
  const isAutoGeneratedContainer = a.type === ActivityTypeEnum.PARALLEL || a.type === ActivityTypeEnum.SEQUENCE

  if (!isAutoGeneratedContainer && (a.id !== b.id || a.name !== b.name)) {
    return false
  }

  if (a.type === ActivityTypeEnum.CONDITION && b.type === ActivityTypeEnum.CONDITION && a.condition !== b.condition) {
    return false
  }

  const aChildren = getNestedChildren(a)
  const bChildren = getNestedChildren(b)
  return aChildren.every((arr, i) => isStructurallyEqual(arr, bChildren[i]))
}
  ...
  try {
    ...
    for (const activity of nested) {
      ...
      const hasOutgoingEdges = edges.some(
        (e) =>
          e.source === activity.id &&
          (e.sourceHandle === EdgeHandleEnum.TRUE || e.sourceHandle === EdgeHandleEnum.FALSE)
      )
