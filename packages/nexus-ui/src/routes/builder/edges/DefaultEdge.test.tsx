...
describe('DefaultEdge with hover', () => {
  ...
  it('renders with hover mock setup', async () => {
    // Note: vi.doMock after imports doesn't reliably override due to ESM module caching.
    // This test verifies the component renders without errors when re-imported.
    // Full hover behavior (EdgeActions visibility) is covered by E2E/integration tests.
    vi.doMock('./useEdgeHandlers', () => ({
      useEdgeHandlers: () => ({
        isHovered: true,
        isEdgeHovered: true,
        effectiveMarkerEnd: 'url(#arrow)',
    ...
    const { DefaultEdge: HoveredDefaultEdge } = await import('./DefaultEdge')
    const { container } = render(<HoveredDefaultEdge {...defaultProps} />)
