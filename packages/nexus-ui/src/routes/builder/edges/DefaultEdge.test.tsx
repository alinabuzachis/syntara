...
describe('DefaultEdge', () => {
  ...
  it('renders EdgePath', () => {
    render(<DefaultEdge {...defaultProps} />)
  ...
  it('renders EdgeLabel when label is provided', () => {
    render(<DefaultEdge {...defaultProps} label="Test Label" />)
  ...
  it('shows EdgeActions when data.isActive is true', () => {
    render(<DefaultEdge {...defaultProps} data={{ isActive: true }} />)
  ...
  it('hides EdgeActions when isPending is true', () => {
    render(<DefaultEdge {...defaultProps} data={{ isActive: true, isPending: true }} />)
    expect(screen.queryByTestId('edge-actions')).not.toBeInTheDocument()
  })

  it('hides EdgeActions when executionStatus is set', () => {
    render(<DefaultEdge {...defaultProps} data={{ isActive: true, executionStatus: 'passed' }} />)
    expect(screen.queryByTestId('edge-actions')).not.toBeInTheDocument()
  })
})

describe('DefaultEdge with nodesConnectable=false', () => {
  afterEach(() => {
  ...
  it('hides edge actions when nodesConnectable is false even if hovered', () => {
...
describe('DefaultEdge with hover', () => {
  const defaultProps = {
    id: 'edge-1',
    source: 'node-1',
    target: 'node-2',
  ...
  it('hides EdgeActions when hovered but isPending', () => {
    mockIsHovered.value = true
    render(<DefaultEdge {...defaultProps} data={{ isPending: true }} />)
    expect(screen.queryByTestId('edge-actions')).not.toBeInTheDocument()
  })

  it('hides EdgeActions when hovered but has executionStatus', () => {
    mockIsHovered.value = true
    render(<DefaultEdge {...defaultProps} data={{ executionStatus: 'pending' }} />)
    expect(screen.queryByTestId('edge-actions')).not.toBeInTheDocument()
  })
})
