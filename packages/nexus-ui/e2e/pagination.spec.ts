...
function workflowBuilderLinks(app: Page) {
  return app.getByRole('grid', { name: 'Workflows table' }).locator('tbody a[href*="/workflow-builder/"]')
}
...
test.describe('Pagination Footer — Users Tab', () => {
  ...
  test('pagination footer is visible with per-page toggle', async ({ app }) => {
    const pagination = app.getByRole('navigation', { name: 'Pagination' })
...
test.describe('Pagination Footer — Groups Tab', () => {
  test('pagination footer is visible on groups tab', async ({ app }) => {
    ...
    await expect(app.getByRole('navigation', { name: 'Pagination' })).toBeVisible()
  })
})
...
test.describe('Project Selector — Workflows', () => {
  ...
  test('selecting a project sends project_id to the API', async ({ app }) => {
    const workflowCountAllProjects = await workflowBuilderLinks(app).count()
    test.skip(
    ...
    const workflowCountFiltered = await workflowBuilderLinks(app).count()
...
test.describe('Pagination Navigation — Workflows', () => {
  test.beforeEach(async ({ app }) => {
    ...
    test.skip(!hasNextPage, 'Not enough data to paginate — need more than 20 workflows')
  })
