...
class TestMixedScopes:
    ...
    def test_read_workflows_project_a(self, mixed_scopes_env):
        resp = mixed_scopes_env["user_api"].projects.list_workflows(project_id=mixed_scopes_env["proj_a_id"])
        assert resp.status_code == HTTPStatus.OK
        assert resp.parsed is not None
        assert len(resp.parsed.resources) >= 1

    def test_read_workflows_project_b(self, mixed_scopes_env):
        resp = mixed_scopes_env["user_api"].projects.list_workflows(project_id=mixed_scopes_env["proj_b_id"])
        assert resp.status_code == HTTPStatus.OK
        assert resp.parsed is not None
        assert len(resp.parsed.resources) >= 1

    def test_read_credentials_project_a(self, mixed_scopes_env):
        resp = mixed_scopes_env["user_api"].credentials.list()
        assert resp.status_code == HTTPStatus.OK
        assert resp.parsed is not None
        assert len(resp.parsed.resources) >= 1
