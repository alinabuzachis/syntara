...
class TestGlobalRoles:
    ...
    def test_can_read_workflows_project_a(self, global_roles_env):
        resp = global_roles_env["user_api"].projects.list_workflows(
    ...
    def test_can_read_credentials_project_a(self, global_roles_env):
        resp = global_roles_env["user_api"].credentials.list()
        assert resp.status_code == HTTPStatus.OK
        assert resp.parsed is not None
        assert len(resp.parsed.resources) >= 1
    ...
    def test_role_has_system_scope(self, global_roles_env):
        resp = global_roles_env["admin_api"].roles.list()
        assert resp.status_code == HTTPStatus.OK
        assert resp.parsed is not None
        matching = [r for r in resp.parsed.resources if r.name == global_roles_env["role_name"]]
