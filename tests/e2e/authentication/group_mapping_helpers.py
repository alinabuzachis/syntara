...
def create_nexus_group(nexus_api: NexusApiRegistry, name: str) -> UUID:
    ...
    resp = nexus_api.groups.create(body=GroupCreate(name=name))
    assert resp.status_code == HTTPStatus.CREATED, f"create group failed: {resp.status_code}"
    assert resp.parsed is not None
    group_id = resp.parsed.id
...
def get_user_id_by_username(nexus_api: NexusApiRegistry, username: str) -> UUID:
    ...
    resources = list_resp.parsed.resources
...
def list_user_groups(nexus_api: NexusApiRegistry, user_id: UUID) -> list[UserGroupRead]:
    ...
    resp = nexus_api.users.list_groups(user_id=user_id)
    assert resp.status_code == HTTPStatus.OK
    assert resp.parsed is not None
    return list(resp.parsed.resources or [])
...
def set_allow_all_authenticated(
    ...
    *,
    ...
    get_resp = nexus_api.identity_providers.get(provider_id=provider_id)
    assert get_resp.status_code == HTTPStatus.OK, f"get IdP failed: {get_resp.status_code}"
    assert get_resp.parsed is not None
    config = get_resp.parsed.configuration
    assert isinstance(config, OIDCConfigurationResponse)

    patch_resp = nexus_api.identity_providers.patch(
    ...
    assert patch_resp.status_code == HTTPStatus.OK, f"patch IdP failed: {patch_resp.status_code} {patch_resp.content!r}"
