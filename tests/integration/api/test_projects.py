...
async def test_create_project_assigns_admin(
    ...
    response = await auth_client.get(f"/api/v1/projects/{project_id}/roles")
    assert response.status_code == 200
    assignments = response.json()
    assert len(assignments) == 1
    assert assignments[0]["user_id"] == str(test_user.id)
...
async def test_project_admin_grants_roles(
    ...
    response = await auth_client.post(
        ...
        json={"user_id": str(other_user.id), "role_name": "project-user"},
    )
    assert response.status_code == 201
    ...
    response = await auth_client.post(
        f"/api/v1/projects/{project_id}/roles",
        json={"user_id": str(other_user.id), "role_name": "project-auditor"},
    )
    assert response.status_code == 201
...
async def test_project_admin_grants_admin_to_another_user(
    ...
    response = await auth_client.post(
        f"/api/v1/projects/{project_id}/roles",
        json={"user_id": str(other_user.id), "role_name": "project-admin"},
    )
    assert response.status_code == 201
...
async def test_assigned_user_gets_project_scoped_policies(
    ...
    assert "workflow:read:any" in project_policy_names
    assert "workflow:create:any" in project_policy_names
    assert "execution:read:any" in project_policy_names
...
async def test_revoke_project_role(
    ...
    # Revoke it
    response = await auth_client.delete(f"/api/v1/projects/{project_id}/roles/{assignment_id}")
...
async def test_invalid_role_name_rejected(
    ...
    response = await auth_client.post(
        f"/api/v1/projects/{project_id}/roles",
        json={"user_id": str(other_user.id), "role_name": "superadmin"},
    )
...
async def test_non_admin_cannot_assign_roles(
    ...
    response = await auth_client.post(
        f"/api/v1/projects/{project_id}/roles",
        json={"user_id": str(non_admin.id), "role_name": "project-user"},
    )
    assert response.status_code == 201

    # Verify non_admin does NOT have project-role:assign
    ...
    for p in project_policies:
        all_actions.update(p["actions"])
    assert "project-role:assign" not in all_actions

    # Switch auth to non_admin and try to assign a role — should be denied
    _auth_as(non_admin)
    response = await auth_client.post(
        f"/api/v1/projects/{project_id}/roles",
        json={"user_id": str(target_user.id), "role_name": "project-user"},
    )
...
async def test_project_auditor_has_read_only_permissions(
    ...
    response = await auth_client.post(
        f"/api/v1/projects/{project_id}/roles",
        json={"user_id": str(auditor.id), "role_name": "project-auditor"},
    )
    assert response.status_code == 201
...
async def test_project_admin_assigns_group_role(
    ...
    response = await auth_client.get(f"/api/v1/projects/{project_id}/group-roles")
    assert response.status_code == 200
    assignments = response.json()
    assert len(assignments) == 1
...
async def test_revoke_group_role_removes_access(
    ...
    # Revoke
    response = await auth_client.delete(f"/api/v1/projects/{project_id}/group-roles/{assignment_id}")
    assert response.status_code == 204
