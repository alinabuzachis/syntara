...
async def test_file_upload_events_logged() -> None:
    ...
    with patch("nexus.agent_orchestrator.context_manager.file_manager.logger") as mock_logger:
        file_manager = FileManager()
...
async def test_storage_failure_detailed_logging() -> None:
    ...
    with (
        patch("nexus.agent_orchestrator.context_manager.file_manager.logger") as mock_logger,
        patch("nexus.agent_orchestrator.context_manager.file_manager.get_settings", return_value=custom_settings),
    ):
        file_manager = FileManager()
