...
_check_payload_size) and the receive_webhook / receive_eda_webhook endpoints
...
class TestReceiveWebhookEndpoints:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize(("endpoint_fn", "trigger_type", "label", "default_path"), _ENDPOINT_PARAMS)
    async def test_payload_validation_called(
        self, endpoint_fn: Callable[..., Any], trigger_type: str, label: str, default_path: str
    ) -> None:
        """_validate_payload is called with trigger and payload."""
        mock_db = AsyncMock(spec=AsyncSession)
        trigger = _make_trigger(webhook_path=default_path)
        execution_id = uuid4()
    ...
    async def test_invalid_payload_raises_validation_error(self) -> None:
        """Invalid payload raises TriggerValidationError (uses real validation)."""
        mock_db = AsyncMock(spec=AsyncSession)
