...
class TestMSWordConverterConversion:
    ...
    @pytest.mark.asyncio
    @patch(
        "nexus.agent_orchestrator.context_manager.file_manager.document_conversion.converters.ms_word_converter.pypandoc.convert_file"
    )
    @patch(
        "nexus.agent_orchestrator.context_manager.file_manager.document_conversion.converters.ms_word_converter.tempfile.NamedTemporaryFile"
    )
    ...
    @pytest.mark.asyncio
    @patch(
        "nexus.agent_orchestrator.context_manager.file_manager.document_conversion.converters.ms_word_converter.Path.unlink"
    )
    @patch(
        "nexus.agent_orchestrator.context_manager.file_manager.document_conversion.converters.ms_word_converter.Path.exists"
    )
    @patch(
        "nexus.agent_orchestrator.context_manager.file_manager.document_conversion.converters.ms_word_converter.pypandoc.convert_file"
    )
...
class TestMSWordConverterWithRealFiles:
    ...
    def fixtures_dir(self) -> Path:
        ...
        return Path(__file__).parent.parent.parent.parent.parent.parent.parent / "fixtures" / "files"
