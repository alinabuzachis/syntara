  ...
  -d @- <<EOF
{
  "name": "simple-data-processing",
  "description": "Process customer data with external tool",
  "yaml_definition": $(cat simple-data-processing.yaml | jq -Rs .)
}
EOF
```
  ...
  -d @- <<EOF
{
  ...
  "yaml_definition": $(cat expense-approval.yaml | jq -Rs .)
}
EOF
```
  ...
  -d @- <<EOF
{
  "name": "nexus-generated-workflow",
  "description": "Workflow automatically generated by Nexus service",
  "yaml_definition": $(cat nexus-workflow.yaml | jq -Rs .)
}
EOF
```
...
{
  "name": "code-analysis-workflow",
  "yaml_definition": $(cat code-analysis.yaml | jq -Rs .)
}
EOF
```
...
{
  "name": "failure-recovery-test",
  "yaml_definition": $(cat failure-recovery.yaml | jq -Rs .)
}
EOF
```
